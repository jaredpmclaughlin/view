...
#include <GL/glew.h>
#include <GLFW/glfw3.h> // used instead of GLUT
...
namespace view
{
  ...
  class Solid 
  {				// failing to come up with a good name for this class
 ...
 */
  ...
  protected:
    std::unique_ptr<std::vector<GLfloat>> vert_buf_data;
  ...
  public:
    ...
    std::unique_ptr<std::vector<GLfloat>> getBuf();
  };

  class STest : public Solid {
    public:
    STest();
  };
...
*/
  ...
  public:
    ...
    //   void GLBuffer();
    void setBuf(std::unique_ptr<std::vector<GLfloat>>);
...
main (int argc, char **argv)
{
  ...
  {
    std::cerr << "error: " << e.error () << " for arg " << e.
      argId () << std::endl;
  }

  // an instance of the test object just for getting things together
  std::unique_ptr<view::Solid> object = std::make_unique<view::STest>();
  view::Window mainWindow;

  if (mainWindow.init () == EXIT_FAILURE)
    return EXIT_FAILURE;

  mainWindow.loop (object.get());
...
view::Solid::Solid () {
    this->vert_buf_data = std::make_unique<std::vector<GLfloat>>();
};
...
int
view::Solid::getBufferSize ()
{
 return this->vert_buf_data->size () * sizeof (GLfloat);
}

int
view::Solid::getVerticeCount ()
{
  return this->vert_buf_data->size () / 3;	// 3D space
}

std::vector < GLfloat > *view::Solid::getBuffer ()
{
  return (this->vert_buf_data).get();
}

std::unique_ptr<std::vector<GLfloat>> view::Solid::getBuf(){
    return move(this->vert_buf_data);
};

/*
void
view::Window::setBuffer (std::vector < GLfloat > *vertex_buffer)
{
  this->vertex_buffer_data = vertex_buffer;
}
*/
void
view::Window::setBuf(std::unique_ptr<std::vector<GLfloat>> vert_buff) {
      this->vert_buf_data = std::move(vert_buff);
}

int
view::Window::getBufferSize ()
{
  return this->vert_buf_data->size () * sizeof (GLfloat);
}

int
view::Window::getVerticeCount ()
{
  return this->vert_buf_data->size () / 3;	// 3D space
}

void
view::Window::GLBuffer ()
{
  ...
  glBufferData (GL_ARRAY_BUFFER,
		sizeof (GLfloat) * (this->vert_buf_data->size ()),
		this->vert_buf_data->data (), GL_STATIC_DRAW);
...
void
view::Window::drawLines (int vertices)
{
...
void
view::handle_args (int argc, char **argv)
{
  ...
  TCLAP::ValueArg < std::string > dummyArg ("d", "dummy", "A dummy arg.",
					    false, "homer", "string");
  // add the argument
  cmd.add (dummyArg);

  // Here's a switch, this version adds itself, so you don't need the above call
  TCLAP::SwitchArg dummySwitch ("s", "switch", "Switch the switch.", cmd,
				false);
